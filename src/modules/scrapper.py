...
google_url = 'https://www.google.com/search?q=distância entre '
...
class ScrapFromFile(SheetHandler):
        ...
        try:
            async with self.session.get(url, headers=headers) as response:
                ...
                if response.status == 429:
                    print('Received 429 status code. Creating a new session.')
                    await self._close_session()
                    await self._create_session()
    ...
    def _export(self):
        ...
        distance_col: List[float | str | None] = [
            # Lista que será utilizada para criar a coluna de distâncias no dataframe.
        ]
